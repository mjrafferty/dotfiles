        ...
        if [[ -d ~/dotfiles/ ]]; then
            if [[ $(stat -c '%a' ~/dotfiles/) -ne 700 ]]; then
                chmod 700 ~/dotfiles;
						...
						rsync -q --chmod=o-rwx -rpt --exclude '.git' --exclude 'login_server' ~/dotfiles/ -e "ssh -q -i ${HOME}/.ssh/nex$(whoami).id_rsa -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PasswordAuthentication=no" nex$(whoami)@$1:~/ && ssh -i ~/.ssh/nex$(whoami).id_rsa -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PasswordAuthentication=no nex$(whoami)@$1;
