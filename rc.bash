...
sum_traffic(){
	...
	if [[ $1 == '-d' ]]; then 
		DECOMP='zgrep' 
		DATE="-$(date --date="-$2 day" +%m%d%Y).zip"; 
		shift; 
		shift; 
	else 
		DECOMP='grep' 
...
googlemx(){
	...
	if [[ $1 == --list ]]; then
		echo; 
		(echo 'ID Domain'; sudo -u $(getusr) -- siteworx -u -n -c Dns -a listZones | awk '($2 !~ /nextmp/) {print $1,$2}') | column -t; 
		echo
	elif [[ $1 = '-c' && -n $2 ]]; then
		zoneid=$(sudo -u $(getusr) -- siteworx -u -n -c Dns -a listZones | awk "(\$2 ~ /$2/)"'{print $1}')
		echo -e "\nID ... MX-Records-for: $2"
		(sudo -u $(getusr) -- siteworx -u -n -c Dns -a queryDnsRecords --zone_id $zoneid) | awk '($4 ~ /MX/) {print $1,$4,$6,$7,$8}' | sort -nk3 | column -t; 
		echo
	fi
